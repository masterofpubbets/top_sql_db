...
GO
ALTER PROC [dbo].[sp_TOPPendingItems]
@username nvarchar(50)
AS
SELECT
[TOP],[TOTAL % DONE],[PLAN HO DATE],[Target Date],[Responsible]
,vDetails.Discipline,vDetails.Item,vDetails.[Item Description]
FROM
(

SELECT 
[index],[WAR ROOM PRIORITY],[COMM BLOCK],[COMMISSIONING PRIROITY],[TOP],[PLAN HO DATE],[Target Date],[LATE DATE],[Subsystem description],[Resp.],
...
SELECT 'REINSTATED' as Discipline
,[top_id]
,[ht_name] as Item
,null as [Item Description]
FROM [dbo].[tblHT] WITH (NOLOCK)
where reinstated_date is null
...
SELECT 'EQUIPMENT' as Discipline
,[top_id]
,[tag] as Item
,[description] as [Item Description]
FROM [dbo].[tblEquipment] WITH (NOLOCK)
where erected_date is null

---------------------------------------------------------------------------------------------------------

UNION ALL

SELECT 'CABLE LAYING' as Discipline
,[top_id]
,[tag] as Item
,[description] as [Item Description]
FROM [dbo].[tblCables] WITH (NOLOCK)
where pulled_date is null

---------------------------------------------------------------------------------------------------------

UNION ALL

SELECT 'CABLE TERMINATION' as Discipline
,[top_id]
,[tag] as Item
,[description] as [Item Description]
FROM [dbo].[tblCables] WITH (NOLOCK)
where con_from_date is null or con_to_date is null

---------------------------------------------------------------------------------------------------------

UNION ALL

SELECT 'INSTRUMENT' as Discipline
,[top_id]
,[tag] as Item
,[description] as [Item Description]
FROM [dbo].[tblInstruments] WITH (NOLOCK)
...
SELECT 'HOOKUP' as Discipline
,[top_id]
,[tag] as Item
,[description] as [Item Description]
FROM [dbo].[tblInstruments] WITH (NOLOCK)
where [hookup_require]=1
and hookup_date is null
...
ON vSelectedTOP.[index] = vDetails.top_id
INNER JOIN tblTOPSelected ON vSelectedTOP.[TOP] = tblTOPSelected.top_name
WHERE tblTOPSelected.[user_name] = @username
GO
