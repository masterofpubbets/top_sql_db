ALTER PROC sp_getPunchesPendingAByMember
AS
...
FROM (SELECT DISTINCT CASE WHEN [Responsible] = 'Not Assigned' THEN '000_000Not Assigned' ELSE [Responsible] END AS [Responsible] FROM #tempPunch WHERE [Category] = 'A' AND [Cons. Closed Date] IS NULL AND [Offical CLosed Date] IS NULL) as x order by Responsible
...
set @result = '
            ...
            FROM (
                ...
                pivot 
                (
                    COUNT([ID]) FOR [Responsible] in (' + @col + ')
                ) p 
            ) AS memberPending
