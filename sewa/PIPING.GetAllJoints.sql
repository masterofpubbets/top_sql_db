...
,ISOS.[ISO Key] + '-' + JointNumber + (CASE WHEN JoinName IS NULL THEN '' ELSE '-' + JoinName END) AS [Joint Key]
...
,CASE WHEN ((VisualAccept=0) OR (VisualAccept IS NULL)) THEN 'NO' ELSE 'YES' END AS [Visual Accepted]
...
,CASE WHEN ((PWHTAccepted=0) OR (PWHTAccepted IS NULL)) THEN 'NO' ELSE 'YES' END AS [PWHT Accepted]
,HTaNumber AS [HTA Number],HTDate AS [HT Date],HTChkValue AS [HT%]
,CASE WHEN ((HTAccepted=0) OR (HTAccepted IS NULL)) THEN 'NO' ELSE 'YES' END AS [HT Accepted]
,RT1UT1Number AS [RT1UT1 Number],RT1UT1Date AS [RT1UT1 Date],RTUTChkValue AS [RTUT%]
,CASE WHEN ((RTUTAccepted=0) OR (RTUTAccepted IS NULL)) THEN 'NO' ELSE 'YES' END AS [RTUT Accepted]
...
,CASE WHEN ((PMIAccepted=0) OR (PMIAccepted IS NULL)) THEN 'NO' ELSE 'YES' END AS [PMI Accepted]
,MTNumber AS [MT Number],MTDate AS [MT Date],MTPTChkValue AS [MTPT%]
,CASE WHEN ((MTPTAccepted=0) OR (MTPTAccepted IS NULL)) THEN 'NO' ELSE 'YES' END AS [MTPT Accepted]
,PTTCNumber AS [PTTC Number],PTDate AS [PT Date],PTTCChkValue AS [PTT%]
,CASE WHEN ((PTTCAccepted=0) OR (PTTCAccepted IS NULL)) THEN 'NO' ELSE 'YES' END AS [PTTC Accepted]
